        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h3 class="text-lg font-semibold mb-4">{{ __('Podsumowanie importu') }}</h3>
                    <p class="mb-6 text-gray-600 dark:text-gray-400">
                        {{ __('Poniżej znajduje się lista zmian, które zostaną wprowadzone po zatwierdzeniu. Sprawdź, czy wszystko się zgadza.') }}
                    </p>

                    @if(!empty($changes['updated']))
                        <div class="mb-6">
                            <h4 class="font-bold text-yellow-600 dark:text-yellow-400">{{ __('Rekordy do zaktualizowania') }} ({{ count($changes['updated']) }}):</h4>
                    ...
                    @if(!empty($changes['created']))
                        <div class="mb-6">
                            <h4 class="font-bold text-blue-600 dark:text-blue-400">{{ __('Rekordy do utworzenia') }} ({{ count($changes['created']) }}):</h4>
                    ...
                    @if(!empty($changes['no_change']))
                        <div class="mb-6">
                            <h4 class="font-bold text-gray-500 dark:text-gray-400">{{ __('Rekordy bez zmian') }} ({{ count($changes['no_change']) }}):</h4>
                    ...
                    @if(empty($changes['updated']) && empty($changes['created']))
                         <div class="mt-4 p-4 bg-gray-100 dark:bg-gray-700 rounded-md">
                            <p class="text-gray-600 dark:text-gray-300 italic">{{ __('Brak zmian do zaimportowania. Dane w pliku są identyczne z danymi w systemie.') }}</p>
                         </div>
                    @endif
